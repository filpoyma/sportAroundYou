...
_Step 1:_ Create a _Procfile_ with the following line: `web: yarn start:prod`. We do this because Heroku runs `yarn start` by default, so we need this setting to override the default run command.
...
_Step 1:_ Run `npm install` to install dependencies, then `yarn build` to create the `./build` folder.
...
_Step 3:_ Run `PUBLIC_PATH='/web-app/' BUILD_FOLDER_PATH='build/web-app' yarn build`, to save production build inside `./build/web-app` folder.
...
_Step 1:_ Add AWS EB start scripts in _package.json_: `"aws-eb:prod": "yarn build && yarn start:prod"`
